App = {
  ...
  initContract: function() {
    $.getJSON('contracts/MCTokenCrowdsale.json', function(data) {
  ...
  initCoin: function() {
    $.getJSON('contracts/MCToken.json', function(data) {
      // Get the necessary contract artifact file and instantiate it with truffle-contract.
      var MCTokenArtifact = data;
      App.contracts.MCToken = TruffleContract(MCTokenArtifact);

      // Set the provider for our contract.
      App.contracts.MCToken.setProvider(App.web3Provider);
  ...
  handleSendTransaction: function(event) {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.Crowdsale.deployed().then(function(instance) {
        crowdsaleInstance = instance;
        //发送交易
        var result = crowdsaleInstance.sendTransaction({ from: account, value: web3.toWei(amount/1000, "ether")});
        console.log(result)
        return result;
  ...
  getBalances: function() {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      MCTokenInstance = App.contracts.MCToken.at(App.tokenAddress);

      MCTokenInstance.balanceOf(account).then(function(result) {
